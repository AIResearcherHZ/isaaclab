...
* :obj:`SEMI_TAKS_LV1_CFG`: Semi-body Taks_LV1 closed-chain robot (parallel waist + dual arms with linkage wrists, fixed base)
...
# Semi-body Taks_LV1 (闭链) USD路径
_SEMI_TAKS_LV1_USD_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "taks", "Semi_Taks_LV1.usd"
)
...
##
# Semi-body Closed-chain Configuration
##

SEMI_TAKS_LV1_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path=_SEMI_TAKS_LV1_USD_PATH,
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False,
            retain_accelerations=False,
    ...
    actuators={
        "waist": ImplicitActuatorCfg(
            ...
            velocity_limit_sim=4.19,
            stiffness={
                "waist_yaw_joint": 589.409607,
                "waist_(right|left)_motor_joint": 1178.82,
            },
            damping={
                "waist_yaw_joint": 37.522984,
                "waist_(right|left)_motor_joint": 75.04,
            },
            armature=0.149299,
        ),
        "arms": ImplicitActuatorCfg(
        ...
        "wrists": ImplicitActuatorCfg(
            joint_names_expr=[
                ".*_wrist_roll_joint",
                ".*_arm_long_link_motor_joint",
                ".*_arm_short_link_motor_joint",
            ],
            effort_limit_sim=7.0,
            velocity_limit_sim=20.944,
            stiffness={
                ".*_wrist_roll_joint": 6.632374,
                ".*_arm_(long|short)_link_motor_joint": 13.264748,
            },
            damping={
                ".*_wrist_roll_joint": 0.422230,
                ".*_arm_(long|short)_link_motor_joint": 0.844460,
            },
            armature=0.001680,
        ),
        "passive_linkage": ImplicitActuatorCfg(
            joint_names_expr=[
                "waist_roll_joint",
                "waist_pitch_joint",
                "waist_(right|left)_link_driven_joint.*",
                ".*_arm_long_link_active_joint",
                ".*_arm_short_link_active_joint",
                ".*_arm_long_link_bevel_gear_joint",
                ".*_arm_short_link_bevel_gear_joint",
                ".*_wrist_yaw_joint",
                ".*_wrist_pitch_joint",
            ],
            stiffness=0.0,
            damping=0.01,
            armature={
                "waist_(roll|pitch)_joint": 0.149299,
                "waist_(right|left)_link_driven_joint.*": 0.149299,
                ".*_arm_(long|short)_link_(active|bevel_gear)_joint": 0.001680,
                ".*_wrist_(yaw|pitch)_joint": 0.001680,
            },
        ),
    },
)
"""Semi-body Taks_LV1 closed-chain robot configuration for manipulation tasks.

Fixed base, parallel-linkage waist and linkage-driven wrists (MJCF equality
constraints imported as loop spherical joints + PhysX mimic joints), no legs/neck.
Actuated joints (17): waist_yaw, waist_right/left_motor, shoulder_pitch/roll/yaw
and elbow per arm, wrist_roll and arm_long/short_link_motor per arm.
Passive linkage joints: waist_roll/pitch, arm_long/short active + bevel_gear,
wrist_yaw/pitch (driven through parallel loops and mimic couplings).
"""