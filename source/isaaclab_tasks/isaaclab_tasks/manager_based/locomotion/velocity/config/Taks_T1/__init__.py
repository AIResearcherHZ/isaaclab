...
gym.register(
    ...
    kwargs={
        # 环境配置入口：指向当前模块下的 rough_env_cfg 中的 TaksT1RoughEnvCfg
...
# 注册粗略地形（试玩模式）版本的 Taks_T1 机器人速度控制任务环境
gym.register(
    id="Isaac-Velocity-Rough-Taks_T1-Play-v0",
    entry_point="isaaclab.envs:ManagerBasedRLEnv",
    disable_env_checker=True,
    kwargs={
        # 环境配置入口：试玩模式下使用 TaksT1RoughEnvCfg_PLAY
...
# 注册平坦地形（试玩模式）版本的 Taks_T1 机器人速度控制任务环境
gym.register(
    id="Isaac-Velocity-Flat-Taks_T1-Play-v0",
    entry_point="isaaclab.envs:ManagerBasedRLEnv",
    disable_env_checker=True,
    kwargs={
        # 环境配置入口：试玩模式下使用 TaksT1FlatEnvCfg_PLAY
