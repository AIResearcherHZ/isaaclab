...
parser.add_argument(
    ...
    help=(
        ...
        " --no-flatten is set. Off by default so the .usda/payloads remain for inspection or editing."
    ),
)
parser.add_argument(
    "--no-flat-hierarchy",
    ...
    help=(
        ...
        " moves joints under <root>/joints. Default-on so prim_paths like {ENV_REGEX_NS}/Robot/<link>"
        " resolve under Isaac Lab's non-recursive token matcher."
    ),
)
parser.add_argument(
    "--keep-empty-scopes",
    ...
    default=False,
    help=(
        "After flattening the hierarchy, keep intermediate scopes (Geometry/Physics/...) even if they"
        " only contain non-rigid placeholder prims (e.g. URDF 'world' link). Off by default to match"
        " the clean <Robot>/<link> layout shown by Omniverse Stage."
    ),
)
...
import omni.usd  # noqa: F401 — needed so the local-scope rule below doesn't trip on the `omni` name

import isaaclab.sim as sim_utils
from isaaclab.utils.assets import check_file_path

# isaacsim 5.1 ships two URDF importers:
#   - 2.4.31+109.0.1 (C++ .so, ABI-broken against USD 25.11 runtime) — what
#     isaaclab.sim.converters.UrdfConverter hardcodes
#   - 3.2.1 (pure-Python rewrite via urdf_usd_converter) — the one we use here
# We bypass isaaclab's UrdfConverter and call the new importer directly. The
# 3.2.1 API has no equivalent for the legacy CLI flags, so they're applied
# post-hoc on the produced USD: --fix-base is realized by
# _anchor_chain_root_to_world in the flatten pass; --merge-joints / --joint-*
# still need to be applied as USD edits if needed.
...
#
# URDF importer 3.2.1 emits the kinematic tree as a nested chain under
...
def _anchor_chain_root_to_world(stage, root_path, joints_scope_path) -> bool:
    ...
    link (e.g. a `world` link from URDF) as the anchor when present.
    """
...
def _flatten_hierarchy(usd_path: str, *, drop_empty_scopes: bool = True, fix_base: bool = False) -> dict:
    ...
    # exactly one body rel is empty. The 3.2.1 importer's root joint has body0=<root>
    # and body1=<base-link> — clear body0 to match the convention fix_root_link expects.
    for joint_name in joint_names:
        jp = stage.GetPrimAtPath(joints_scope_path.AppendChild(joint_name))
...
def main():
    ...
    # The 3.2.1 importer writes <out_dir>/<robot_name>/<robot_name>.usda; we additionally
    ...
    # CLI flags kept for backwards compat — 3.2.1 has no equivalent, must be applied post-hoc.
    # --fix-base is now applied via _anchor_chain_root_to_world in the flatten pass.
    unsupported = []
    if args_cli.merge_joints:
        unsupported.append("--merge-joints")
    if args_cli.joint_stiffness != 100.0 or args_cli.joint_damping != 1.0 or args_cli.joint_target_type != "position":
        unsupported.append("--joint-*")
    if unsupported:
        print(
            f"[WARN] convert_urdf.py: the following flags are ignored under URDF importer 3.2.1 "
    ...
    importer = URDFImporter(cfg)
    produced_usda = importer.import_urdf()
    ...
    try:
        ...
        if args_cli.no_flatten:
            ...
            # The importer publishes physics overrides through a Physics variant set. Force
            # selection here so Flatten() composes the physx variant (CollisionAPI, RigidBodyAPI,
            # ArticulationRootAPI, drives) into the final layer.
